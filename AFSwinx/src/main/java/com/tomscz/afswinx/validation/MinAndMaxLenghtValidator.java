...
/**
 * This validator represent minimal or maximal length validator.
 * 
            ...
            if (valueToValidate == null || valueToValidate.length() < this.value) {
                throw new ValidationException(
                        LocalizationUtils.getTextValueFromLocalOrExtendBundle(
                                AFSwinxLocaleConstants.VALIDATION_LENHTH_TO_SMALL, localization)
                                + this.value);
            }
        } else {
            if (valueToValidate != null && valueToValidate.length() > this.value) {
                throw new ValidationException(
                        LocalizationUtils.getTextValueFromLocalOrExtendBundle(
                                AFSwinxLocaleConstants.VALIDATION_LENGTH_TO_GREAT, localization)
                                + this.value);
            }
        }
    }
