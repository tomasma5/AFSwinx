...
public class AFScreenDefinitionBuilder {
    ...
    public AFAndroidProxyScreenDefinition getScreenDefinition() throws Exception {
        ...
        if (components != null) {
            for (int i = 0; i < components.length(); i++) {
                ...
                SupportedComponents componentType = (SupportedComponents) AFRestUtils.getEnumFromString(
                        SupportedComponents.class,
                        component.getString(COMPONENT_TYPE_KEY),
                        true
                );
    ...
    private String retrieveScreenDefinition() throws Exception {
        if (url != null) {
            ...
            RequestTask task = new RequestTask(context, url)
